// Package renewal decides WHEN a certificate should be renewed.
//
// Its own package because internal/api needs exactly one function from it —
// DefaultRenewalPolicy, to tell an agent when to come back — and importing
// internal/agent for that linked all 7,301 lines of it: nebula, gvisor,
// wireguard, miekg/dns, os/exec. Everything here depends on time and a hash.
package renewal
...
func (p Policy) RenewAt(notBefore, notAfter time.Time, seed string) time.Time {
	if p.Fraction <= 0 {
		p.Fraction = 0.5
	}
...
// to prevent one.
func (p Policy) RenewAtWithHint(notBefore, notAfter time.Time, seed string, hint time.Time) time.Time {
	...
	if spread <= 0 {
		spread = DefaultPolicy().PullForwardSpread
	}
...
// certificate alone.
func (p Policy) Assess(now, notBefore, notAfter time.Time, seed string) Urgency {
...
func (p Policy) AssessWithHint(now, notBefore, notAfter time.Time, seed string, hint time.Time) Urgency {
	switch {
