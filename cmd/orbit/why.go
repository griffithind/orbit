...
func whyCmd(ctx context.Context, args []string) error {
	...
	var (
		...
		asJSON  = fs.Bool("json", false, "emit the raw explanation")
	)
	// parseFlags, not fs.Parse: this is the only diagnostic command that takes
	// an operand, and Go's flag package stops at the first non-flag — so
	...
	if fs.NArg() != 1 {
		return usageErrorf("usage: orbit why <peer> [-proto tcp] [-port 443]\n\n" +
			"<peer> is an overlay address, or the name of a peer with a tunnel.")
	}
	...
	ex, err := agent.FetchExplain(ctx, path, slug, agent.ExplainRequest{
		Peer: fs.Arg(0), Proto: *proto, Port: *port,
	})
	if err != nil {
...
func printDecision(r renderer, label string, d agent.Decision, prep string) {
	switch {
	case d.Allowed:
