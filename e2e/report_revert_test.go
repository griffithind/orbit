...
func (h *harness) reportAs(t *testing.T, membershipID uuid.UUID, r store.AgentReport) {
	t.Helper()
	err := h.store.Tx(context.Background(), func(ctx context.Context, tx *store.Tx) error {
		return tx.RecordAgentReport(ctx, membershipID, r)
	})
	if err != nil {
...
func (h *harness) appliedEpochs(t *testing.T, membershipID uuid.UUID) (int64, int64) {
	t.Helper()
	var cfg, blk int64
...
func TestRevertLowersTheRecordedEpochOnlyWhenNamed(t *testing.T) {
	...
	h.reportAs(t, membershipID, store.AgentReport{ConfigEpoch: 5, BlocklistEpoch: 3, AgentVersion: "e2e"})
	if cfg, blk := h.appliedEpochs(t, membershipID); cfg != 5 || blk != 3 {
	...
	h.reportAs(t, membershipID, store.AgentReport{ConfigEpoch: 2, BlocklistEpoch: 1})
	if cfg, blk := h.appliedEpochs(t, membershipID); cfg != 5 || blk != 3 {
	...
	h.reportAs(t, membershipID, store.AgentReport{
	...
	h.reportAs(t, membershipID, revert)
	if cfg, blk := h.appliedEpochs(t, membershipID); cfg != 4 || blk != 2 {
	...
	h.reportAs(t, membershipID, store.AgentReport{ConfigEpoch: 6, BlocklistEpoch: 4})
	if cfg, blk := h.appliedEpochs(t, membershipID); cfg != 6 || blk != 4 {
...
func TestRevertIsAudited(t *testing.T) {
	...
	h.reportAs(t, membershipID, store.AgentReport{ConfigEpoch: 5, BlocklistEpoch: 3})
	// A forward report writes no audit entry: only regressions are events.
	h.reportAs(t, membershipID, store.AgentReport{ConfigEpoch: 6, BlocklistEpoch: 3})
	...
	h.reportAs(t, membershipID, revert)
	h.reportAs(t, membershipID, revert) // replayed: no second entry, since nothing regressed
...
func TestRevertShowsUpInConvergence(t *testing.T) {
	...
	current, _ := h.appliedEpochs(t, membershipID)
	current++ // stand in for a freshly pushed generation

	h.reportAs(t, membershipID, store.AgentReport{ConfigEpoch: current})
	if !h.convergedOn(t, ts.URL, membershipID, current) {
...
func (h *harness) convergedOn(t *testing.T, baseURL string, membershipID uuid.UUID, epoch int64) bool {
	t.Helper()
	...
	for _, l := range conv.Lagging {
		if l.MembershipID == membershipID.String() {
