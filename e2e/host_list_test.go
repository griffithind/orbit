...
func (h *harness) listHosts(t *testing.T, baseURL, query string) wire.HostListResponse {
	t.Helper()
	var out wire.HostListResponse
	u := baseURL + "/v1/hosts?network_id=" + h.netID.String()
	if query != "" {
		u += "&" + query
	}
...
func listedNames(resp wire.HostListResponse) []string {
	out := make([]string, 0, len(resp.Hosts))
...
func TestHostListPagesAndFilters(t *testing.T) {
	...
	for i, name := range []string{"alpha", "bravo", "charlie", "delta"} {
		...
		if i < 2 {
			req.Tags = append(req.Tags, "front")
		}
		if code := h.adminPost(t, ts.URL+"/v1/hosts", req, nil); code != http.StatusCreated {
	...
	if second.NextCursor != "" {
		...
		t.Fatalf("full final page handed out a cursor that returned %d hosts", len(third.Hosts))
	}
	...
	if len(counted.Hosts) != 2 {
		t.Errorf("page size = %d, want 2 — the count must not widen the page", len(counted.Hosts))
	}
	...
	if got := h.listHosts(t, ts.URL, "state=suspended"); len(got.Hosts) != 0 {
		t.Errorf("state=suspended returned %d hosts, want none", len(got.Hosts))
	}
}
...
func TestHostListBehindFilter(t *testing.T) {
	...
	var caughtUp, behind wire.HostResponse
	...
	for _, hostID := range []string{caughtUp.ID, behind.ID} {
		id := uuid.MustParse(hostID)
		h.setState(t, id, store.HostActive)
	}
...
func (h *harness) setState(t *testing.T, hostID uuid.UUID, state string) {
	t.Helper()
	err := h.store.Tx(context.Background(), func(ctx context.Context, tx *store.Tx) error {
		return tx.SetHostState(ctx, hostID, state)
	})
	if err != nil {
...
func TestHostResponseCarriesWhatPatchAccepts(t *testing.T) {
	...
	if page.Hosts[0].RoleName != "default" || len(page.Hosts[0].StaticAddrs) != 1 {
		t.Errorf("listed host is missing role_name or static_addrs: %+v", page.Hosts[0])
	}
	...
	var updated wire.HostResponse
	if code := h.adminReq(t, http.MethodPatch, ts.URL+"/v1/hosts/"+created.ID,
...
func TestHostListRejectsParametersItCannotHonour(t *testing.T) {
	...
	for _, q := range []string{
		...
		code := h.adminReq(t, http.MethodGet,
			ts.URL+"/v1/hosts?network_id="+h.netID.String()+"&"+q, nil, &out)
		if code != http.StatusBadRequest {
			t.Errorf("GET /v1/hosts?%s = %d, want 400", q, code)
		}
	}
}
